...
const StaffManagementPage = () => {
  ...
  useEffect(() => {
    const nextTotalPages = Math.max(1, Math.ceil(staffList.length / PAGE_SIZE));
    if (page > nextTotalPages) {
      setPage(nextTotalPages);
    }
  }, [staffList.length, page]);
  ...
  const pagedStaffList = staffList.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
