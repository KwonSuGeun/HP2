...
import type { StaffRegisterForm } from "@/features/staff/StaffTypes";
import { DEFAULT_REGISTER_FORM, DEPARTMENT_OPTIONS } from "@/features/staff/formConstants";
import {
  isMedicalStaff,
  isValidKoreanMobilePhone,
  KOREAN_MOBILE_PHONE_INVALID_MESSAGE,
} from "@/features/staff/staffFormUtils";
import { formatDaumBaseAddress } from "@/lib/daumPostcode";
import AddressSearchDialog from "@/components/staff/AddressSearchDialog";
...
type StaffRegisterModalProps = {
  ...
  loading?: boolean;
  onClose: () => void;
  onSubmit: (form: StaffRegisterForm) => void;
};

type FormErrors = Partial<Record<keyof StaffRegisterForm, string>>;
...
/** 신규 직원 등록 모달 */
const StaffRegisterModal = ({
  open,
  loading = false,
  onClose,
  onSubmit,
}: StaffRegisterModalProps) => {
  const [form, setForm] = useState<StaffRegisterForm>(DEFAULT_REGISTER_FORM);
  ...
  const updateField = <K extends keyof StaffRegisterForm>(
    key: K,
  ...
  const validate = (): boolean => {
    ...
    if (!form.staffId.trim()) nextErrors.staffId = "사번은 필수입니다.";
                ...
                <section className={styles.registerFormSection}>
                  ...
                  <div className={styles.registerFieldGrid}>
                    <div className={styles.registerField}>
                      <FieldLabel label="사번" required htmlFor="staffId" />
                      <input
                        id="staffId"
                        type="text"
                        className={`${styles.input} ${errors.staffId ? styles.inputError : ""}`}
                        placeholder="사번을 입력하세요"
                        value={form.staffId}
                        onChange={(event) => updateField("staffId", event.target.value)}
                      />
                      {errors.staffId ? <p className={styles.fieldError}>{errors.staffId}</p> : null}
                    </div>
